...
JUNOD_NODE="http://localhost:44039"
KEY="acc0"
KEYRING="test"
...
curl http://localhost:8080/ --include --header "Content-Type: application/json" -X POST --data '{"chain-id":"localjuno-1","action":"bin","cmd":"tx gov vote 4 yes --yes --gas=500000 --fees 500ujuno --node %RPC% --home %HOME% --chain-id %CHAIN_ID% --from validator --keyring-backend=test"}'

junod q gov proposals